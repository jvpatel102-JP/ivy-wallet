...
import com.ivy.data.sync.GoogleDriveSyncService
...
class SettingsViewModel @Inject constructor(
    ...
    private val googleSyncEnabled = mutableStateOf(false)
    private val googleAccountEmail = mutableStateOf<String?>(null)
    private val lastSyncTime = mutableLongStateOf(0L)
    ...
    override fun uiState(): SettingsState {
        ...
        return SettingsState(
            ...
            googleSyncEnabled = googleSyncEnabled.value,
            googleAccountEmail = googleAccountEmail.value,
            lastSyncTime = lastSyncTime.longValue,
    ...
    private fun initializeGoogleSync() {
        googleSyncEnabled.value = googleDriveSyncService.isGoogleAccountConnected()
        googleAccountEmail.value = googleDriveSyncService.getConnectedEmail()
        lastSyncTime.longValue = sharedPrefs.getLong(GoogleDriveSyncService.PREF_LAST_SYNC_TIME, 0L)
    }
    ...
    override fun onEvent(event: SettingsEvent) {
        when (event) {
            ...
            is SettingsEvent.ConnectGoogleAccount -> connectGoogleAccount(event.email)
            SettingsEvent.DisconnectGoogleAccount -> disconnectGoogleAccount()
    ...
    private fun connectGoogleAccount(email: String) {
        sharedPrefs.putBoolean(GoogleDriveSyncService.PREF_GOOGLE_DRIVE_SYNC_ENABLED, true)
        sharedPrefs.putString(GoogleDriveSyncService.PREF_GOOGLE_ACCOUNT_EMAIL, email)
        googleSyncEnabled.value = true
